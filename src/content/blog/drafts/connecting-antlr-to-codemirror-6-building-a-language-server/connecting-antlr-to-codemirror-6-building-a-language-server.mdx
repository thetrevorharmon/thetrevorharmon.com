...
Lezer reads the grammar files as well the referenced `./highlight` file and generates a parser based on those files. The extension (parser) is then passed to CodeMirror, and CodeMirror can now read and parse JSON.

However, there are cases where writing a Lezer grammar doesn’t make sense. For example, if you have already written a grammar and it powers more than just a front end code editor experience, it may be more favorable to connect an existing language server to CodeMirror instead of maintaining two separate implementations. I found myself in such a case when working on the editor for ShopiyQL, which has a grammar in ANTLR.
...
In order to demonstrate how to get CodeMirror and ANTLR connected, I decided it would be easiest to write a tiny language to use as an example.
...
We can use ANTLR to generate parser and lexer files based off the Zephyr grammar definition. ANTLR has a handful of runtimes; we’ll be using the javascript version to build files for us. There is an excellent [`antlr4ts`](https://github.com/tunnelvisionlabs/antlr4ts) package that we can use to generate both a language server and the associated types. First we need to download our packages:
