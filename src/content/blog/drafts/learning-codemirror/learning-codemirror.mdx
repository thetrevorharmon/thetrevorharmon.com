...
If you have a value that you need to track that is static or fits into the limits of the [facet's API](https://codemirror.net/docs/ref/#state.Facet^define^config), a facet may be the right choice. However, if you have a dynamic value that tracks some kind of state, you may be better off with a state field.
...
State fields are computed every time the state goes through an update cycle, and each time the state field has an opportunity to recompute and provide a new value.
...
Each transaction contains an array of effects, and we can loop through them looking for our value. When the call to `effect.is()` returns true, the type for `effect.value` is inferred to be of type `number` (and therefore compatible with the `+=` operation).

In addition to tracking custom state, a state field can provide information to the view to know how to render the value contained in the state field. Let's say we want to display the count in a panel beneath the editor. We can utilize the state field's [`provide`](https://codemirror.net/docs/ref/#state.StateField^define^config.provide) argument to return an extension that recieves the value from the state field and renders the value in a panel.

Let's first make a small helper function to create the DOM element with the correct content:
...
CodeMirror's `EditorView` class manages and updates the DOM as the state changes. There are times when you may want to manipulate the DOM in some way, but because the view manages it, any changes you make manually0 will be discarded when a state update cycle occurs and new version of the DOM is rendered.

One of CodeMirror's answers to this problem is the `Decoration` class, which enables you to manipulate the DOM in harmony with the view. There are four types of decorations that CodeMirror offers:
