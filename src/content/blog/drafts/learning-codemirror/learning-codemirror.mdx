...
In July 2022, I was tasked with integrating [CodeMirror](https://codemirror.net/), a “code editor component for the web”, into [ShopifyQL Notebooks](https://shopify.engineering/building-commerce-data-models-with-shopifyql). I had never used CodeMirror before, much less worked with implementing code editors, and I found learning about CodeMirror an uphill battle. The library was more complex than I anticipated, and there were many times that I came away from reading the docs scratching my head. After spending many months staring at [the docs](https://codemirror.net/docs/) and [discussion board](https://discuss.codemirror.net/), I have a clearer picture of how CodeMirror works. My goal with this article is to explain certain core concepts and give commentary in addition to the many [examples](https://codemirror.net/examples/) that already exist on the CodeMirror site.
...
<Callout>
  ...
  read](https://codemirror.net/docs/), too.
</Callout>

## Structure & lifecycle

CodeMirror is organized into:

1. Core functionality
2. Added functionality through extensions

Rather than trying to bundle all of the editor concerns into a single, massive package, it enables the developer to pick and choose the functionality they want. Within the core functionality, there are two key pieces:
...
In order to keep updates to the state very fast, updates to the editor state are pure–they do not produce side effects. A view may produce a side effect as a result of the change in state, but the state is very focused with re-computing changes to the state.

The typical lifecyle for an update within CodeMirror looks like this (this is from the **State and Updates** section of the [system guide](https://codemirror.net/docs/guide/)):
...
Functionality beyond the "core" functionality lives inside of extensions. `Extension` is a bit of a catch-all term within CodeMirror, and I think of them as “ways I can add other functionality to an editor”. CodeMirror offers a lot of functionality through extensions–here is a brief list of some of the things it can do:
...
This really only scratches the surface, but is meant to highlight the wide range of possible extensions that CodeMirror provides. I would recommend looking at the [reference guide](https://codemirror.net/docs/ref/) to see all of the extensions that CodeMirror offers.

## CodeMirror primitives

Many of the extensions offered by CodeMirror are "pre-built"–internally they take CodeMirror primitives and create an extension with a single configuration point. However, there are times when you need to craft your own for custom functionality. Some primitives you will most likely encounter are:
...
I imagine a CodeMirror facet sort of like adding a value to a javascript object. Let's say that we represent the editor state as a simple object (the state doesn't actually look like this):
...
However, this approach wouldn't scale very well as our needs become more complex. Additionally, it requires us to add every possible option to our editor state object, which means that even if our editor is very basic, it has to contain all of the functionality of a very complex editor.

CodeMirror's solution to this problem is the [`Facet`](https://codemirror.net/docs/ref/#state.Facet) class–it gives you a named value (like a member of a class) but also provides ways to take multiple inputs and produce a single output.

The simplest way to use a facet is to use its `of` method which takes a value and returns an extension that passes that value to the editor state. For example, if you want to set the tab size or set the editor to read only, you can use facets that are static methods on the `EditorState` class:
...
CodeMirror uses facets in multiple ways to provide extensibility. As shown before, they can be as trivial as a configuration value like `tabSize`. Other facets provide "hooks" into CodeMirror's internal workings. For example, if you wanted to set up a function that logged a message to the console every time the editor changed focused, you could use the view [`updateListener`](https://codemirror.net/docs/ref/#view.EditorView^updateListener) facet:
...
There are even other ways to use a facet. Some return classes that display DOM elements, like [`showTooltip`](https://codemirror.net/docs/ref/#view.showTooltip) or [`showPanel`](https://codemirror.net/docs/ref/#view.showPanel). You use these functions in a similar way as the other functions, with the `of` method:
...
You first define a facet (without a value), then create an extension from a value, and then you can read it out of the state with `state.facet()`.
...
If you have value that you need to track that is static or fits into the limits of the [facet's API](https://codemirror.net/docs/ref/#state.Facet^define^config), a facet may be the right choice. However, if you have a value that tracks some kind of state, you may be better off with a state field.

### StateField

[The docs](https://codemirror.net/docs/ref/#state.StateField) describe a state field as:

> Fields can store additional information in an editor state, and keep it in sync with the rest of the state.

State fields are computed every time the state goes through an update cycle, and each time the state field has an opportunity to recompute and provide a new value. The two main ingredients of a state field are:
...
There are times when we want a state field to track a change emitted by some other field or extension. For that, we can use a [`StateEffect`](https://codemirror.net/docs/ref/#state.StateEffect). The docs explain:
...
State effects are a way to wrap and pass a value between primitives. Because it requires you to create a class instance before using it to wrap a value, it enables a very clear way to identify a given value. Let's imagine that we want to update our state field to also increase the counter by a value in a state effect. We first need to create the state effect:
...
With this code, we have created a state effect that increments our counter by 5. Now that we can dispatch this state effect to update our counter, we need to update the state field to accept new values from this effect:
...
Each transaction contains an array of effects, and we can loop through them looking for our value. When the call to `effect.is()` returns true, the type for `effect.value` is automatically determined for us to be of type `number` (and therefore compatible with the `+=` operation).

In addition to tracking custom state field, a state can provide information to the view to know how to render the value in the state field. Let's say we want to display the count in the bottom-right corner of the editor. We can utilize the [`provide`](https://codemirror.net/docs/ref/#state.StateField^define^config.provide) argument to return an extension that recieves the value from the state field and that we can use to render a piece of DOM.

Let's first make a small helper function to create the DOM element with the correct text:
...
const changeCounterStateField = StateField.define<number>({
  ...
  provide: (value) => showPanel.from(value, createCounterPanel), // highlight-line
});
```

The `provide` function passes the field's value and expects an `Extension` in return. Here we're using the [`showPanel`](https://codemirror.net/docs/ref/#view.showPanel) facet to return an extension that takes the value and creates a panel with the desired content. You can see a working example of this below:

import {StateFieldExample} from './StateFieldExample';

<StateFieldExample />

As a final example, let's make a new state field that consumes a state effect that CodeMirror exports. Imagine that we want to keep a running list of all of the linter errors (called `Diagnostic`) in our editor. We can make use of the [`setDiagnosticsEffect`](https://codemirror.net/docs/ref/#lint.setDiagnosticsEffect) state effect type that CodeMirror exposes to us. The code to track the diagnostics would look like this:

```ts
import {setDiagnosticsEffect, Diagnostic} from '@codemirror/lint';
...
CodeMirror's `EditorView` class manages and updates the DOM as the `EditorState` changes. There are times when you may want to manipulate the DOM in some way, but because the view manages it, any changes you may manually make will be discarded when a state update cycle occurs and new version of the DOM is created.

CodeMirror's answer to this problem is the `Decoration` class, which enables you to manipulate the DOM in various ways. There are four types of decorations that CodeMirror offers:

1. [Mark decorations](https://codemirror.net/docs/ref/#view.Decoration^mark): These enable you to modify a range of content (like adding a classname). For example, CodeMirror uses mark decorations to provide syntax highlighting.
...
4. [Line decorations](https://codemirror.net/docs/ref/#view.Decoration^line): Similar to mark decorations, these enable you to modify content, but it modifies things on a line basis.

The `EditorView` class has a static facet called [`decorations`](https://codemirror.net/docs/ref/#view.EditorView^decorations) that enable us to provide decorations to the view.
...
A view plugin enables us to draw additional content in the editor in a way that isn't tied to a specific range. It receives updates when the view is redrawn which allows a developer to determine how the view plugin should be re-drawn/updated.
...
This create method does the following:

1. Creates a new element.
2. Uses the [`view.dom`](https://codemirror.net/docs/ref/#view.EditorView.dom) property to access the underlying HTML element and appends our new element to the dom. It also assigns the reference to that element to our private `widget` property.
...
We will also add an update and destroy method to handle changes to the editor:
