...
slug: connecting-an-antlr-language-server-to-codemirror-6
date: 2023-05-24
type: Post
status: Draft
...
_Note: this article is the second of a two part series. The previous article covers topics like Lezer, ANTLR, a toy language called Zephyr, and how to make an ANTLR-based language server. If you're unfamiliar with those topics, you may want to start with the first article._
...
Before implementing the rest of our parser adapter, let’s take a closer look at Lezer's parse tree. Lezer’s [`Tree` class](https://lezer.codemirror.net/docs/ref/#common.Tree) has a static [build method](https://lezer.codemirror.net/docs/ref/#common.Tree^build) that we can use to build our own tree. We can provide it with either a [`BufferCursor`](https://lezer.codemirror.net/docs/ref/#common.BufferCursor) or just [an array of integers](https://lezer.codemirror.net/docs/ref/#common.Tree^build^data.buffer) that correspond to the shape of the a. When we provide it with a buffer of integers, Lezer reads the buffer in groups of 4 integers, where each group of four integers corresponds to a single node. Here’s what each position in the group means:
...
The start and end offsets for the first two tokens correspond to the position of that node relative to the top of the document, and each has the typical size of 4. However, the top node has a start offset of 0 and an end offset of 11. As the parent of both of the previous nodes, it needs to encompass both of those nodes’ offsets. Additionally, the size of the top node is 12–4 for the first node, 4 for the second node, and 4 for itself (making 12 in total).
...
CodeMirror uses the [`Language`](https://codemirror.net/docs/ref/#language.Language) class to describe a code language–it includes a lezer parser, language data, the language's name, and any additional extensions associated with the language. Language data describes features of the language in a way that CodeMirror understands, and if done correctly, will provide certain code editing features "for free" (since the features are generic enough that CodeMirror has a pattern around the particular feature). For example, you can describe the way the language denotes comments or what kind of brackets it uses.
...
In order to connect our parser adapter to CodeMirror, we can use the `Language` class inside of a `LanguageSupport` class. Here's an example of what this might look like:
...
2. We create a new language. The first argument is an empty `Facet` (we'll cover why this is necessary in the Language Data section) and the second argument is our parser adapter. We're passing an empty array for the `extraExtensions` argument, and passing the name `Zephyr` so that the language has a name.
...
If we pass the `zephyr` (langauge support) object as an extension within CodeMirror, CodeMirror will then be able to parse our own language.

## Syntax highlighting

Even though CodeMirror may understand the language, it still doesn't have proper support for syntax highlighting. We can use the [`syntaxHighlighting`](https://codemirror.net/docs/ref/#language.syntaxHighlighting) function that CodeMirror provides to do most of the work for us–but we do need to tell CodeMirror a little bit more about our language to make that work.

Lezer contains a [highlight module](https://lezer.codemirror.net/docs/ref/#highlight) that connects a node with style information by using the [`Tag`](https://lezer.codemirror.net/docs/ref/#highlight.Tag) class. The docs describe the class as:
...
```tsx
const tokenToNodeType: {[key in Token | 'topNode']: NodeType} = {
  topNode: NodeType.define({id: 0, name: 'topNode', top: true}),
  const: NodeType.define({id: 1, name: 'const'}),
  let: NodeType.define({id: 2, name: 'let'}),
  semicolon: NodeType.define({id: 3, name: 'semicolon'}),
...
We need to use two methods to map these node types to Lezer tags:
...
In order to tell CodeMirror more about our language, we can pass language data when we create our language. [The docs](https://codemirror.net/docs/ref/#:~:text=literal%20dollar%20sign.-,languageDataAt,-%3CT%3E) give the following examples of language data:
...
Typically we might pass in language data via the first argument of the `Language` class (the `data` argument). However, since we are maintaining our own node set, [we need to take a different approach](https://discuss.codemirror.net/t/language-data-not-being-recognized-on-custom-language/6310/1). We should instead provide language data via the top node's [props](https://lezer.codemirror.net/docs/ref/#common.NodeType^define^spec.props) argument. This is why we passed an empty facet to our call to `Language` earlier–any data passed there will be unused.
...
Providing this data to CodeMirror will give us editor features for "free"–for example, the `commentTokens` make it so that the default [toggle comment command](https://codemirror.net/docs/ref/#commands.defaultKeymap) works with our language!
